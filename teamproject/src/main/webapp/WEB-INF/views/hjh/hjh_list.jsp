			...
			for(j in Menu)
				$(".list_menu").eq(i).append(Menu[j]+"<input type='checkbox' value='"+Menu[j]+"'>");
				 ...
				 if($(this).is(':checked')==true){
					 $(this).parent().parent().find('table').append(
					 ...
					 tr.eq(tr.length-1).find('input').on('click',function(){
							...
							if($(this).val() == '+'){
								...
								temp.eq(1).text(10000*count);
								 map_list.set(temp.eq(0).text(),count);
								list_add(10000);
								}
							// - 누르면
							else{
...
<body>
	<div style="width: 400px;height: 680px;float: left; margin-top:10px;">
		...
		<div><table style="width: 400px;height: 60px;"><tr><td>총합</td><td id ='list_add'>0</td><td><input type="button" value="주문" onclick="list_buy()"></td></tr><col width='10%'><col width='80%'><col width='10%'></table> </div>
	</div>
</body>
</html>